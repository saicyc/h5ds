	...
	*/
	var backgroundLayer = function(){
		...
		$("#layerBg").on("click",function(){
			...
			//背景参数渲染 。。。。
			v.backgroundImage = allObj.obj.$layer.css("background-image");
			v.backgroundColor = allObj.obj.$layer.css("background-color");
			v.backgroundRepeat = allObj.obj.$layer.css("background-repeat");
			...
			var bgHtml = '	<div class="page-background-set"><h5>底&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;色：</h5><input type="color" id="layerBackgroundColor" list="layerBackgroundColor_list" value="{{backgroundcolor}}">'+
                       ...
                       '         <span placeholder="选择下拉" data="repeat" id="playCount_data">平铺</span>'+
                       ...
                       '         </ul>'+
                       '    </div></div>';
			bgHtml = bgHtml.replace("{{backgroundcolor}}",v.backgroundColor.colorHex());
			$("#basicSetBox").html(bgHtml);
			
			var html = allObj.obj._pageBackground;
			//html = html.replace("{{backgroundcolor}}",v.backgroundColor.colorHex());
			html = html.replace("{{url}}",v.backgroundImage == "none" ? "":v.backgroundImage.split("(")[1].split(")")[0]);
			...
			$clear_layerBackgroundColor.on("click",function(){
				$layerBackgroundColor.val("#000000");
				allObj.obj.$layer.css("background-color","initial");	
				ifun.ifun_callback();
			});
			...
			var callback_background = function(data){
				//console.log(data);
				allObj.obj.$layer.css("background-repeat" , data);
